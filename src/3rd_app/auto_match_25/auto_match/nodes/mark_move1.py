...
class MarkNav():
    ...
    def navigation1(self, mark_name):
        ...
        quat = tf_transformations.quaternion_from_matrix(self.drop_position)
        self.currrent_position = self.get_currect_pose()
        tran1 = tf_transformations.translation_from_matrix(self.currrent_position)
        quat1 = tf_transformations.quaternion_from_matrix(self.currrent_position)
        pose = Pose(Point(tran1[0], tran[1], tran[2]), Quaternion(0,0,0,1))
        goal.target_pose.pose = pose
        # 把目标位置发送给MoveBaseAction的服务器
        self.move_base.send_goal(goal)

    #导航到对应的x值
    def navigation2(self, mark_name):
        '''
        根据地点进行导航
        '''
        print("start navigation")
        ...
        tran1 = tf_transformations.translation_from_matrix(self.currrent_position)
        quat1 = tf_transformations.quaternion_from_matrix(self.currrent_position)
        pose = Pose(Point(tran[0], tran1[1], tran[2]), Quaternion(0,0,0,1))
        goal.target_pose.pose = pose
        # 把目标位置发送给MoveBaseAction的服务器
        self.move_base.send_goal(goal)
