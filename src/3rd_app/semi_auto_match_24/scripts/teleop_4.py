...
def keyboardLoop():
    ...
    run_vel_, run_an_ = 0.5, 2.6
    # 低速移动参数
    low_vel_, low_an_ = 0.05, 0.3
    # 2key movement param
    ...
    window_id = hex(pygame.display.get_wm_info()['window'])
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if 1 :
                    if event.key == pygame.K_k and 1:
                        grasp_pub('backward')
                    elif event.key == pygame.K_8 and 1:
                        grasp_pub('forward')
                    elif event.key == pygame.K_l and 1:
                        grasp_pub('right')
                    elif event.key == pygame.K_j and 1:
                        grasp_pub('left')
                    elif event.key == pygame.K_7 and 1:
                        grasp_pub('up')
                    elif event.key == pygame.K_9 and 1:
                        grasp_pub('down')
                        ...
                        grasp_pub('open')
                    elif event.key == pygame.K_q and 1:
                        grasp_pub('arm_left')
                    elif event.key == pygame.K_e and 1:
                        grasp_pub('arm_right')
                ...
                #动作指令集
                if 1 :
                    if event.key == pygame.K_SEMICOLON :  # 抓取0
                        grasp_pub('catch')
        ...
        if 1:
            ...
            if event.type == pygame.KEYDOWN:
                if event.key == pygame.K_LSHIFT:
                    if(vel, an_vel == run_vel_, run_an_):
                        vel, an_vel = low_vel_, low_an_
                        print('vel to low')
                if event.key==pygame.K_CAPSLOCK:
                    if(vel, an_vel == low_vel_, low_an_):
                        vel, an_vel = run_vel_, run_an_
                        print('vel to run')
